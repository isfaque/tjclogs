...
commonQuery.updateDocument = function updateOneDocument(
  model,
  updateCond,
  updateData
) {
  return new Promise(function (resolve, reject) {
    model
      .findOneAndUpdate(updateCond, updateData, {
        new: true,
      })
      .lean()
      .exec(function (err, updatedData) {
...
commonQuery.updateDocumentWithProjection = function updateOneDocument(
  model,
  updateCond,
  updateData,
  projection
...
commonQuery.aggregateAverage = function aggregateAverage(
  model,
  group,
  condition
) {
...
commonQuery.findDataBySortWithPagination = function findDataBySortWithPagination(
  model,
  cond,
  projection,
  sort_by,
  ...
  return new Promise(function (resolve, reject) {
    model
      .find(cond, projection)
      .sort(sort_by)
      .limit(parseInt(page_size))
      .skip(parseInt(page_no * page_size))
      .exec(function (err, data) {
        if (err) {
          reject(err);
        } else {
